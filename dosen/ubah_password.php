...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($pass_lama) || empty($pass_baru) || empty($pass_ulangi)) {
        ...
        $stmt = $pdo->prepare("SELECT password FROM dosen WHERE id = ?");
        ...
        if (!$is_valid) {
            ...
            $pdo->prepare("UPDATE dosen SET password = ? WHERE id = ?")->execute([$new_hash, $user_id]);
...
<head>
    <meta charset="utf-8">
    <title>Ubah Password | Dosen</title>
                    ...
                    <div class="card-body p-0">
                        <form method="POST">
                            <table class="table table-bordered mb-0 form-table">
                                ...
                                <tr><th>Ulangi Password</th><td><input type="password" name="pass_ulangi" class="form-control form-control-sm" required></td></tr>
                            </table>
